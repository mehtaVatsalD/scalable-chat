...
public class RedisRoutingTable implements RoutingTable {
    ...
    private static final String DECREMENT_DELETE_SCRIPT = "local count = redis.call('HINCRBY', KEYS[1], ARGV[1], -1) " +
            "if tonumber(count) <= 0 then " +
            "redis.call('HDEL', KEYS[1], ARGV[1]) " +
            "end " +
            "return count";

    private final RedisScript<Long> disconnectScript = new DefaultRedisScript<>(DECREMENT_DELETE_SCRIPT, Long.class);
