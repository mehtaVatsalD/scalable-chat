...
/**
 * Integration test for Remote (Cross-Server) Message Passing.
 * Verifies that when users are on different servers:
 * 1. The message is published to the target server's Redis topic.
 * 2. The target server receives it and delivers it to the end-user via
 * WebSocket.
 */
public class RemoteMessageScenarioTest {
    ...
    void teardown() {
        ...
        if (nativeRedisClient != null)
            nativeRedisClient.shutdown();
    }
        ...
        try {
            ...
            pubSubConn.addListener(new RedisPubSubAdapter<String, String>() {
                ...
                public void message(String channel, String message) {
                    try {
                        InterNodeChatMessage msg = objectMapper.readValue(message, InterNodeChatMessage.class);
            ...
            // Warm up monitor
            InterNodeChatMessage warmup = new InterNodeChatMessage("system", "system", "WARMUP_SIGNAL", 0);
            ...
            StompSession sessionReceiver = connectStomp(wsUrlB, RECEIVER, receiverMessages);

            // 4. Sender (Server A) sends message to Receiver (Server B)
            ChatMessage chatMsg = new ChatMessage();
            chatMsg.setReceiverIds(Collections.singletonList(RECEIVER));
            chatMsg.setContent("Hello across servers!");
    ...
    private StompSession connectStomp(String url, String userId, BlockingQueue<ClientDeliveryMessage> messageQueue)
            throws Exception {
