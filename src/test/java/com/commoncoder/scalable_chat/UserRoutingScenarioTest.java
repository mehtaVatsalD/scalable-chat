...
/**
 * Plain JUnit test (no @SpringBootTest). The test is a pure observer — it connects to Redis
 * directly and spawns child Spring Boot contexts as the servers under test.
 *
 * <p>Uses Awaitility instead of Thread.sleep() to avoid flaky tests — each assertion actively polls
 * Redis until the expected state arrives or times out.
 */
  ...
  void testUserRoutingCountsOnConnectAndDisconnect() throws Exception {
    ...
    try {
      ...
      await()
          .atMost(WAIT_TIMEOUT)
          .until(
              () -> countForServer(redisTemplate.opsForHash().entries(routingKey), serverAId) == 1);
      log.info(
          "STEP 1 PASSED: 1 connection on Server A → {}",
      ...
      await()
          .atMost(WAIT_TIMEOUT)
          .until(
              () -> countForServer(redisTemplate.opsForHash().entries(routingKey), serverAId) == 2);
      log.info(
          "STEP 2 PASSED: 2 connections on Server A → {}",
          redisTemplate.opsForHash().entries(routingKey));
      ...
      try {
        ...
        await()
            ...
            .until(
                () ->
                    countForServer(redisTemplate.opsForHash().entries(routingKey), serverAId) == 2
                        && countForServer(redisTemplate.opsForHash().entries(routingKey), serverBId)
                            == 1);
        log.info(
            "STEP 3 PASSED: 2 on Server A, 1 on Server B → {}",
            redisTemplate.opsForHash().entries(routingKey));
        ...
        await()
            ...
            .until(
                () ->
                    countForServer(redisTemplate.opsForHash().entries(routingKey), serverAId) == 1);
        log.info(
            "STEP 4 PASSED: Server A decremented to 1 → {}",
            redisTemplate.opsForHash().entries(routingKey));
        ...
        log.info(
            "STEP 5 PASSED: Server A entry removed, Server B still has 1 → {}",
            redisTemplate.opsForHash().entries(routingKey));
        ...
        await()
            .atMost(WAIT_TIMEOUT)
            .until(() -> redisTemplate.opsForHash().entries(routingKey).isEmpty());
        log.info(
            "STEP 6 PASSED: Routing table is empty → {}",
            redisTemplate.opsForHash().entries(routingKey));
