...
/**
 * Integration test for Local Message Passing.
 * Verifies that when two users are on the same server, messages are delivered
 * directly without hitting Redis Pub/Sub.
 */
public class LocalMessageScenarioTest {
    ...
    void testLocalMessageDeliveryFlow() throws Exception {
        ...
        // 1. Boot Server
        ConfigurableApplicationContext server = SpringApplication.run(ScalableChatApplication.class, "--server.port=0");
        ...
        try {
            ...
            // Warm up monitor
            InterNodeChatMessage warmupMsg = new InterNodeChatMessage("system", "system", "WARMUP_SIGNAL", 0);
            ...
            StompSession sessionB = connectStomp(wsUrl, USER_B, userBMessages);

            // 4. Send Message
            ChatMessage msg = new ChatMessage();
            msg.setReceiverIds(Collections.singletonList(USER_B));
            msg.setContent("Hello User B, directly from A!");
